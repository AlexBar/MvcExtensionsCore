...
namespace MvcExtensionsCore
{
    ...
    public class ModelMetadataItemBuilder<TValue> : IModelMetadataItemBuilder<TValue>, IModelMetadataItemConfigurator
    {
        ...
        [NotNull]
        IModelMetadataItemBuilder<TValue> IModelMetadataItemBuilder<TValue>.HtmlEncode(bool endoded)
        {
            return HtmlEncode(endoded);
        }
        ...
        /// <returns></returns>
        [NotNull]
        public ModelMetadataItemBuilder<TValue> HtmlEncode(bool encoded)
        {
            AddAction(m => m.HtmlEncode = encoded);
        ...
        /// <returns></returns>
        [NotNull]
        [Obsolete("Not supported with ASP.NET CORE")]
        ...
        protected virtual ModelMetadataItemBuilder<TValue> Validate<TModel>([NotNull] Func<TModel, TValue, bool> validator, [NotNull] Func<string> errorMessage)
        {
            ...
            return ValidateBy(() => new DelegateBasedValidator((container, value) => validator((TModel)container, (TValue)value))
                                            {
        ...
        [NotNull]
        private static Func<TValidator> CreateFactory<TValidator>() where TValidator : ValidationAttribute
        {
            var validatorType = typeof(TValidator);
            ConstructorInfo constructor = validatorType.GetConstructor(Type.EmptyTypes);

            if (constructor == null)
            {
                throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, "The constructor of {0} is not available.", validatorType));
            }
            
            var @new = Expression.New(constructor);

            return Expression.Lambda<Func<TValidator>>(@new).Compile();
        }
    }
}