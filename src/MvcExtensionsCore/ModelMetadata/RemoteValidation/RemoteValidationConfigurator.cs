...
namespace MvcExtensionsCore
{
    ...
    public class RemoteValidationConfigurator<TValue> : AbstractRemoteValidationConfigurator<TValue>
    {
        ...
        [NotNull] private IRemoteValidationConfigurator<TValue> Core => this;
        ...
        {
            return name.Length > 1 ? char.ToUpperInvariant(name[0]) + name.Substring(1) : name.ToUpperInvariant();
        }
        ...
        private void CreateRemoteValidation<TController>([NotNull] Expression<Func<TController, ActionResult>> action, [AspMvcArea] string areaName, string routeName, [NotNull] IEnumerable<string> additionalFields)
        {
            var controller = typeof(TController).Name.Replace("Controller", string.Empty);
            var methodCall = (MethodCallExpression) action.Body;
        ...
        private void CreateRemoteValidation([AspMvcController] string controller, [AspMvcAction] string action, [AspMvcArea] string areaName, string routeName, [NotNull] IEnumerable<string> additionalFields)
        {
